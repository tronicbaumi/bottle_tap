...
#include "ConvFnc_InitPosDetect_Float64.h"
...
typedef struct {
    ...
    int16 t_pulse;
    int16 t_period;
} INITPOSDETECT_FLOAT64_IMPL_PARAM;

/** Private prototypes */
static uint8 convertM2I(const INITPOSDETECT_FLOAT64_MASK_PARAM *maskParam, INITPOSDETECT_FLOAT64_IMPL_PARAM *impParam, float_CoT ts);
...
/* USERCODE-BEGIN:Conversion                                                                                          */
    ...
    if (error == 0)
    {
        ...
        impParam->u_pulse = (float64)ampPhasor; /* voltage phasor-amplitude */
...
uint8 InitPosDetect_Float64_SaveMP(INITPOSDETECT_FLOAT64 *block, INITPOSDETECT_FLOAT64_MASK_PARAM *maskParam, const uint8 data[], uint16 dataLen)
{
    ...
    else
    {
        ...
        InitPosDetect_Float64_BackupMP(maskParam);
        ...
        error = InitPosDetect_Float64_ConvertMP(block, maskParam);

        /** - Check for errors during execution of save function */
        if (error != 0)
        {
            /* Restore old mask parameter */
            InitPosDetect_Float64_RestoreMP(maskParam);
        }
...
uint8 InitPosDetect_Float64_InitMP(INITPOSDETECT_FLOAT64 *block, const INITPOSDETECT_FLOAT64_MASK_PARAM *maskParam)
{
    ...
    INITPOSDETECT_FLOAT64_IMPL_PARAM *implParam = (INITPOSDETECT_FLOAT64_IMPL_PARAM*)ImplParamBuffer;
    ...
    SaveFncDataBuffer[0] = (uint8)((*(uint64*)&(implParam->u_pulse)) & 0x00000000000000FF);
    SaveFncDataBuffer[1] = (uint8)((*(uint64*)&(implParam->u_pulse) >> 8) & 0x00000000000000FF);
    SaveFncDataBuffer[2] = (uint8)((*(uint64*)&(implParam->u_pulse) >> 16) & 0x00000000000000FF);
    SaveFncDataBuffer[3] = (uint8)((*(uint64*)&(implParam->u_pulse) >> 24) & 0x00000000000000FF);
    SaveFncDataBuffer[4] = (uint8)((*(uint64*)&(implParam->u_pulse) >> 32) & 0x00000000000000FF);
    SaveFncDataBuffer[5] = (uint8)((*(uint64*)&(implParam->u_pulse) >> 40) & 0x00000000000000FF);
    SaveFncDataBuffer[6] = (uint8)((*(uint64*)&(implParam->u_pulse) >> 48) & 0x00000000000000FF);
    SaveFncDataBuffer[7] = (uint8)((*(uint64*)&(implParam->u_pulse) >> 56) & 0x00000000000000FF);
    SaveFncDataBuffer[8] = (uint8)(implParam->t_pulse & 0x00FF);
    SaveFncDataBuffer[9] = (uint8)((implParam->t_pulse >> 8) & 0x00FF);
    SaveFncDataBuffer[10] = (uint8)(implParam->t_period & 0x00FF);
    SaveFncDataBuffer[11] = (uint8)((implParam->t_period >> 8) & 0x00FF);

    /** - Execute save function of block */
    error = InitPosDetect_Float64_Save(block, SaveFncDataBuffer, 12);
