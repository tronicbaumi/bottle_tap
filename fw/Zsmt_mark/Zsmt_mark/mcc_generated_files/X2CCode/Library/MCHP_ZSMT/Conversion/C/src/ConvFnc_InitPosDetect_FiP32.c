...
#include "ConvFnc_InitPosDetect_FiP32.h"
...
typedef struct {
    ...
    int16 t_pulse;
    int16 t_period;
} INITPOSDETECT_FIP32_IMPL_PARAM;

/** Private prototypes */
static uint8 convertM2I(const INITPOSDETECT_FIP32_MASK_PARAM *maskParam, INITPOSDETECT_FIP32_IMPL_PARAM *impParam, float_CoT ts);
...
/* USERCODE-BEGIN:Conversion                                                                                          */
    float_CoT UDCmax, ampPhasor, deltaT, nPulse, nPause, nPeriod;
    
    /** - get mask parameter value */
    UDCmax    = (float_CoT)maskParam->U_max;    /* max DC link voltage */
    ...
    if (UDCmax == (float_CoT)0)
    {
        error += 4; /* Voltage scaling value must not be zero! */
    }
    if (UDCmax < 0)
    {
        UDCmax = -UDCmax;
    ...
    if (error == 0)
    {
        ...
        impParam->u_pulse = getQx32Value(ampPhasor / UDCmax, 31);   /* voltage phasor-amplitude */
...
uint8 InitPosDetect_FiP32_SaveMP(INITPOSDETECT_FIP32 *block, INITPOSDETECT_FIP32_MASK_PARAM *maskParam, const uint8 data[], uint16 dataLen)
{
    ...
    else
    {
        ...
        InitPosDetect_FiP32_BackupMP(maskParam);
        ...
        error = InitPosDetect_FiP32_ConvertMP(block, maskParam);

        /** - Check for errors during execution of save function */
        if (error != 0)
        {
            /* Restore old mask parameter */
            InitPosDetect_FiP32_RestoreMP(maskParam);
        }
...
uint8 InitPosDetect_FiP32_InitMP(INITPOSDETECT_FIP32 *block, const INITPOSDETECT_FIP32_MASK_PARAM *maskParam)
{
    ...
    INITPOSDETECT_FIP32_IMPL_PARAM *implParam = (INITPOSDETECT_FIP32_IMPL_PARAM*)ImplParamBuffer;
    ...
    SaveFncDataBuffer[0] = (uint8)(implParam->u_pulse & 0x000000FF);
    SaveFncDataBuffer[1] = (uint8)((implParam->u_pulse >> 8) & 0x000000FF);
    SaveFncDataBuffer[2] = (uint8)((implParam->u_pulse >> 16) & 0x000000FF);
    SaveFncDataBuffer[3] = (uint8)((implParam->u_pulse >> 24) & 0x000000FF);
    ...
    error = InitPosDetect_FiP32_Save(block, SaveFncDataBuffer, 8);
