...
#include "ConvFnc_EstimatorSwitch_Float32.h"
...
typedef struct {
    float32 thresh_up;
    float32 thresh_down;
} ESTIMATORSWITCH_FLOAT32_IMPL_PARAM;

/** Private prototypes */
static uint8 convertM2I(const ESTIMATORSWITCH_FLOAT32_MASK_PARAM *maskParam, ESTIMATORSWITCH_FLOAT32_IMPL_PARAM *impParam, float_CoT ts);
...
uint8 EstimatorSwitch_Float32_SaveMP(ESTIMATORSWITCH_FLOAT32 *block, ESTIMATORSWITCH_FLOAT32_MASK_PARAM *maskParam, const uint8 data[], uint16 dataLen)
{
    ...
    else
    {
        ...
        EstimatorSwitch_Float32_BackupMP(maskParam);
        ...
        error = EstimatorSwitch_Float32_ConvertMP(block, maskParam);

        /** - Check for errors during execution of save function */
        if (error != 0)
        {
            /* Restore old mask parameter */
            EstimatorSwitch_Float32_RestoreMP(maskParam);
        }
...
uint8 EstimatorSwitch_Float32_InitMP(ESTIMATORSWITCH_FLOAT32 *block, const ESTIMATORSWITCH_FLOAT32_MASK_PARAM *maskParam)
{
    ...
    ESTIMATORSWITCH_FLOAT32_IMPL_PARAM *implParam = (ESTIMATORSWITCH_FLOAT32_IMPL_PARAM*)ImplParamBuffer;
    ...
    error = EstimatorSwitch_Float32_Save(block, SaveFncDataBuffer, 8);
