...
#include "ConvFnc_EstimatorSwitch_Float64.h"
...
typedef struct {
    float64 thresh_up;
    float64 thresh_down;
} ESTIMATORSWITCH_FLOAT64_IMPL_PARAM;

/** Private prototypes */
static uint8 convertM2I(const ESTIMATORSWITCH_FLOAT64_MASK_PARAM *maskParam, ESTIMATORSWITCH_FLOAT64_IMPL_PARAM *impParam, float_CoT ts);
...
/* USERCODE-BEGIN:Conversion                                                                                          */
    float_CoT Thresh_up, Thresh_down;
    ...
    if (error == 0)
    {     
        impParam->thresh_up = Thresh_up;
        impParam->thresh_down = Thresh_down;
    }
...
uint8 EstimatorSwitch_Float64_SaveMP(ESTIMATORSWITCH_FLOAT64 *block, ESTIMATORSWITCH_FLOAT64_MASK_PARAM *maskParam, const uint8 data[], uint16 dataLen)
{
    ...
    else
    {
        ...
        EstimatorSwitch_Float64_BackupMP(maskParam);
        ...
        error = EstimatorSwitch_Float64_ConvertMP(block, maskParam);

        /** - Check for errors during execution of save function */
        if (error != 0)
        {
            /* Restore old mask parameter */
            EstimatorSwitch_Float64_RestoreMP(maskParam);
        }
...
uint8 EstimatorSwitch_Float64_InitMP(ESTIMATORSWITCH_FLOAT64 *block, const ESTIMATORSWITCH_FLOAT64_MASK_PARAM *maskParam)
{
    ...
    ESTIMATORSWITCH_FLOAT64_IMPL_PARAM *implParam = (ESTIMATORSWITCH_FLOAT64_IMPL_PARAM*)ImplParamBuffer;
    ...
    error = EstimatorSwitch_Float64_Save(block, SaveFncDataBuffer, 16);
