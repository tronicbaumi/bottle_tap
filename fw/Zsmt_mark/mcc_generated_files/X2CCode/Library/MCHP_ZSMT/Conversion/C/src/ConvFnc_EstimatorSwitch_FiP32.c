...
#include "ConvFnc_EstimatorSwitch_FiP32.h"
...
typedef struct {
    int32 thresh_up;
    int32 thresh_down;
} ESTIMATORSWITCH_FIP32_IMPL_PARAM;

/** Private prototypes */
static uint8 convertM2I(const ESTIMATORSWITCH_FIP32_MASK_PARAM *maskParam, ESTIMATORSWITCH_FIP32_IMPL_PARAM *impParam, float_CoT ts);
...
/* USERCODE-BEGIN:Conversion                                                                                          */
    ...
    if (error == 0)
    {    
        impParam->thresh_up = getQx32Value(Thresh_up / n_max, 31);
        impParam->thresh_down = getQx32Value(Thresh_down / n_max, 31);
    }
...
uint8 EstimatorSwitch_FiP32_SaveMP(ESTIMATORSWITCH_FIP32 *block, ESTIMATORSWITCH_FIP32_MASK_PARAM *maskParam, const uint8 data[], uint16 dataLen)
{
    ...
    else
    {
        ...
        EstimatorSwitch_FiP32_BackupMP(maskParam);
        ...
        error = EstimatorSwitch_FiP32_ConvertMP(block, maskParam);

        /** - Check for errors during execution of save function */
        if (error != 0)
        {
            /* Restore old mask parameter */
            EstimatorSwitch_FiP32_RestoreMP(maskParam);
        }
...
uint8 EstimatorSwitch_FiP32_InitMP(ESTIMATORSWITCH_FIP32 *block, const ESTIMATORSWITCH_FIP32_MASK_PARAM *maskParam)
{
    ...
    ESTIMATORSWITCH_FIP32_IMPL_PARAM *implParam = (ESTIMATORSWITCH_FIP32_IMPL_PARAM*)ImplParamBuffer;
    ...
    error = EstimatorSwitch_FiP32_Save(block, SaveFncDataBuffer, 8);
