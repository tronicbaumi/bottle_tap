...
#include "ConvFnc_InitPosDetect_FiP16.h"
...
typedef struct {
    ...
    int16 t_pulse;
    int16 t_period;
} INITPOSDETECT_FIP16_IMPL_PARAM;

/** Private prototypes */
static uint8 convertM2I(const INITPOSDETECT_FIP16_MASK_PARAM *maskParam, INITPOSDETECT_FIP16_IMPL_PARAM *impParam, float_CoT ts);
...
/* USERCODE-BEGIN:Conversion                                                                                          */
    ...
    if (error == 0)
    {
        ...
        impParam->u_pulse = getQx16Value(ampPhasor / UDCmax, 15);   /* voltage phasor-amplitude */
...
uint8 InitPosDetect_FiP16_SaveMP(INITPOSDETECT_FIP16 *block, INITPOSDETECT_FIP16_MASK_PARAM *maskParam, const uint8 data[], uint16 dataLen)
{
    ...
    else
    {
        ...
        InitPosDetect_FiP16_BackupMP(maskParam);
        ...
        error = InitPosDetect_FiP16_ConvertMP(block, maskParam);

        /** - Check for errors during execution of save function */
        if (error != 0)
        {
            /* Restore old mask parameter */
            InitPosDetect_FiP16_RestoreMP(maskParam);
        }
...
uint8 InitPosDetect_FiP16_InitMP(INITPOSDETECT_FIP16 *block, const INITPOSDETECT_FIP16_MASK_PARAM *maskParam)
{
    ...
    INITPOSDETECT_FIP16_IMPL_PARAM *implParam = (INITPOSDETECT_FIP16_IMPL_PARAM*)ImplParamBuffer;
    ...
    SaveFncDataBuffer[0] = (uint8)(implParam->u_pulse & 0x00FF);
    SaveFncDataBuffer[1] = (uint8)((implParam->u_pulse >> 8) & 0x00FF);
    SaveFncDataBuffer[2] = (uint8)(implParam->t_pulse & 0x00FF);
    SaveFncDataBuffer[3] = (uint8)((implParam->t_pulse >> 8) & 0x00FF);
    SaveFncDataBuffer[4] = (uint8)(implParam->t_period & 0x00FF);
    SaveFncDataBuffer[5] = (uint8)((implParam->t_period >> 8) & 0x00FF);

    /** - Execute save function of block */
    error = InitPosDetect_FiP16_Save(block, SaveFncDataBuffer, 6);
