...
public class ConvFnc_InitPosDetect_FiP16 extends JavaConversionFunction {
...
// USERCODE-BEGIN:ConvMaskToImplementation                                                                              
        ...
        if (UDCmax == 0) {
            throw new Exception("Voltage scaling value must not be zero!");
        }
        UDCmax = Math.abs(UDCmax);

        // calculate implementation parameter value
        /* scaled voltage phasor amplitude */
        u_pulseCtrVal.setReal(0, 0, QFormat.getQValue(ampPhasor / UDCmax, BITS - 1, BITS, true));
